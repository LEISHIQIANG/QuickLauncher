...
- UI 主题体系新增统一主题解析与自定义窗口外壳入口，普通窗口、工具窗口、弹窗和文件对话框统一走主题控制。
- 插件 API 新增受限 HTTP 请求、插件私有数据目录读写、主程序版本/主题读取、命令执行和动作链处理器注册等稳定接口。
...
- 文件选择、保存和目录选择统一改为非原生 Qt 对话框，并套用项目主题和自定义窗口外壳，避免回退到默认 Windows 窗口。
- C++ DLL 钩子回调从单槽挂起状态改为有界 FIFO 队列，降低高频键鼠事件覆盖回调的风险。
- 插件 HTTP API 收紧请求头、请求体、响应大小、方法和协议边界，减少插件误用或注入风险。
...
- 修复主题链路中父级缺少主题时可能回退到分散样式的问题，统一按父级、应用属性和暗色默认值解析。
- 修复多个弹窗和配置子窗口仍可能使用默认系统窗口边框的问题，统一改为自定义窗口外壳。
- 修复插件搜索源跨插件重名可能互相覆盖的问题，搜索源 ID 自动加插件命名空间并支持注册失败事务回滚。
