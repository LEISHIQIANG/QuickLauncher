...
QuickLauncher 是一款面向 Windows 10 / 11 的桌面快速启动与轻量自动化工具。它以全局弹窗为核心，将应用启动、文件/文件夹访问、URL、热键、命令、批量启动和本地插件整合到同一个搜索入口。
...
- [六、批量启动](#六批量启动)
- [七、插件系统](#七插件系统)
- [八、设置、主题与 UI](#八设置主题与-ui)
- [九、全局 Hook 与输入录制](#九全局-hook-与输入录制)
- [十、数据、安全与更新](#十数据安全与更新)
- [十一、构建、测试与 CI](#十一构建测试与-ci)
- [十二、项目结构](#十二项目结构)
- [十三、维护约定](#十三维护约定)
...
| `macro` | 录制并回放键鼠序列 | `macro_events`、`macro_speed`、`macro_hide_while_recording` |
...
批量启动是独立 `batch_launch` 快捷方式类型。它用于按顺序启动多个已有目标，适合开工环境、一组项目工具或固定排障工具组合。
...
> 动作链（`chain` 快捷方式类型）已从主程序移除；需要按顺序启动多个目标时直接使用 `batch_launch` 即可。
...
信任链基于 SHA-256 校验。哈希缺失或校验失败时，更新验证会失败关闭。
...
│   ├── shortcut_executor.py         # 文件/文件夹/URL/热键/命令/批量启动分发
│   ├── batch_launch_exec.py         # 批量启动运行时
│   └── plugin/                      # 插件包安装、路径、常量
├── modules/                        # 内置模块入口（当前为空，预留扩展）
...
- 文档里的功能数量应来自代码或包清单：快捷方式 7 类、内置命令 33 个、官方插件包 10 个。
