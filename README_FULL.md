...
| **命令系统** | CMD / PowerShell / Python / Git Bash / 内置命令五种模式，50+ 内置命令，命令变量模板，输出捕获，5 层安全预处理 |
| **动作链** | 最多 50 步串行动作，步骤间数据传递，错误中断控制，可取消执行 |
| **外观** | 深色/浅色主题、毛玻璃亚克力效果、自定义图片背景、图标/格子/列数自由配置 |
| **插件** | 权限管理、热加载/卸载、.qlzip 打包安装、自定义命令和搜索源注册 |
| **数据** | 原子保存、自动备份、配置历史（20 快照）、完整备份/恢复 ZIP、可分享配置导出、配置损坏自动恢复 |
| **安全** | 5 层命令预处理管道、路径边界校防目录穿越、SSRF 防护（Favicon 抓取）、命令变量注入防护、ZIP 炸弹防护、符号链接拒绝 |
| **多语言** | 中文（简体）/ English 双语，运行时切换 |
| **自动更新** | GitHub Releases 源、Ed25519 发布签名、SHA-256 校验、后台下载、跳过版本、静默安装 |
...
- **命令子类型**：
  ...
  - `powershell`：PowerShell 脚本执行
  - `python`：Python 脚本执行
  - `git_bash`：Git Bash 命令执行
  - `builtin`：内置应用命令
- **执行模式**：
...
> **注意**：旧版单花括号 `{变量名}` 语法已废弃，配置加载时自动迁移为双花括号 `{{变量名}}`。
...
| `{{lan_ip}}` | 本机默认出口 IPv4 地址 | `192.168.1.100` |
| `{{wan_ip}}` | 公网 IPv4 地址 | `203.0.113.1` |

### 4.2 安全引用（`:q` 后缀）

为防止命令注入，在 CMD/PowerShell/Bash 类型命令中使用外部输入变量时，**必须**添加 `:q` 后缀进行安全引用：
...
```
echo {{{{date}}}}           → 输出: {{date}}（不展开）
echo {{{{clipboard:q}}}}    → 输出: {{clipboard:q}}（不展开）
```
...
命令执行前经过 5 层安全预处理管道，依次执行，任一层阻断则短路终止。

### 5.1 管道阶段

| 层级 | 名称 | 功能 |
|------|------|------|
| 1 | 语法校验 | 输入清洗、命令长度检查（最大 10000 字符） |
| 2 | 语义校验 | 验证命令类型、工作目录是否存在、必填参数非空 |
| 3 | 安全扫描 | 注入检测、变量引用检查、路径安全、危险模式匹配 |
| 4 | 业务规则 | 按快捷方式 ID 执行速率限制 |
| 5 | 审计日志 | 记录校验失败和安全警告 |

### 5.2 安全扫描能力

- **命令注入检测**：识别 `;`、`&`、`|`、`<`、`>`、`` ` `` 等危险字符和链式模式（`&&`、`||`）
- **危险模式匹配**：40+ 正则模式覆盖文件删除、磁盘格式化、注册表操作、进程终止、凭证窃取、横向移动、防御规避等
- **路径穿越防护**：拒绝路径中的 `..`，验证路径在允许的根目录内
- **变量引用强制转义**：外部输入变量必须使用 `:q` 后缀
- **环境变量校验**：检测危险的 `LD_PRELOAD` 式劫持变量
...
|--------|------|
| `enabled` | 是否启用预处理管道 |
| `strict_mode` | 严格模式——安全警告不可覆写 |
| `rate_limiting` | 启用速率限制 |
| `audit_enabled` | 启用审计日志 |
| `block_dangerous_patterns` | 阻断危险模式匹配 |
| `require_variable_quoting` | 强制要求变量引用 |
| `raw_mode` | 跳过变量引用检查（高级用户） |
...
| **CommandDialog** | COMMAND：名称、命令文本、类型（CMD/PowerShell/Python/Git Bash/内置）、参数、环境变量、编码、超时、输出捕获 |
...
| **配置损坏恢复** | `data.json` 损坏时自动从最新备份恢复，生成恢复报告 |

### 13.3 配置修复与恢复

每次启动加载配置时，系统自动执行：

- **语法迁移**：旧版单花括号 `{clipboard}` 自动升级为 `{{clipboard}}`
- **未知变量检测**：识别未注册的变量并报告，不破坏自定义内容
- **损坏隔离**：损坏的配置文件自动隔离，从最新的 5 份自动备份中恢复
- **恢复报告**：恢复详情写入 `recovery/` 目录，包含状态、原因和路径
...
- **导入安全**：ZIP 导入时验证路径穿越、文件大小限制、背景/图标路径白名单

### 13.5 路径安全（path_security）
...
- 默认要求 Ed25519 发布签名；未配置签名公钥时更新校验失败关闭
...
| Ed25519 发布签名 | 使用配置的公钥校验发布签名；空公钥失败关闭 |
...
│   ├── diagnostics.py               #   诊断中心
│   ├── preprocessing/               #   命令预处理管道
│   │   ├── pipeline.py              #     5 层管道编排
│   │   ├── security.py              #     注入检测、危险模式
│   │   ├── validators.py            #     路径/URL/变量校验
│   │   └── ...
│   └── ...                          #   更多核心模块
│
├── ui/                              # 用户界面（~70 模块）
