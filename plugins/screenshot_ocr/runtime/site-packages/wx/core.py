    ...
    if isinstance(item, type):
        ...
        class DeprecatedClassProxy(item):
            def __init__(*args, **kw):
                warnings.warn("Using deprecated class%s. %s" % (name, msg), wxPyDeprecationWarning, stacklevel=2)
        ...
        def deprecated_func(*args, **kw):
            warnings.warn("Call to deprecated item%s. %s" % (name, msg), wxPyDeprecationWarning, stacklevel=2)
            if not kw:
                return item(*args)
            return item(*args, **kw)
        ...
        class DepGetProp:
            ...
            def __get__(self, inst, klass):
                warnings.warn("Accessing deprecated property. %s" % msg, wxPyDeprecationWarning, stacklevel=2)
...
def wxdate2pydate(date):
    ...
    if date.IsValid():
        return datetime.datetime(
            date.year, date.month + 1, date.day, date.hour, date.minute, date.second, date.millisecond * 1000
        )
    else:
        return None
...
def _WindowIDRef___repr__(self):
    return f"WindowIDRef: {self.GetId()}"
...
GridBagSizer.CheckForIntersectionPos = wx.deprecated(
    GridBagSizer.CheckForIntersection, "Use CheckForIntersection instead."
)
...
Window.ConvertDialogPointToPixels = wx.deprecated(
    _Window_ConvertDialogPointToPixels, "Use ConvertDialogToPixels instead."
)
...
Window.ConvertDialogSizeToPixels = wx.deprecated(
    _Window_ConvertDialogSizeToPixels, "Use ConvertDialogToPixels instead."
)
...
def _ToolBar_AddSimpleTool(self, toolId, bitmap, shortHelpString="", longHelpString="", isToggle=0):
    ...
    if isToggle:
        kind = wx.ITEM_CHECK
    return self.AddTool(toolId, "", bitmap, wx.NullBitmap, kind, shortHelpString, longHelpString)
...
def _ToolBar_AddLabelTool(
    ...
    """
    return self.AddTool(id, label, bitmap, bmpDisabled, kind, shortHelp, longHelp, clientData)
...
def _ToolBar_InsertSimpleTool(self, pos, toolId, bitmap, shortHelpString="", longHelpString="", isToggle=0):
    ...
    if isToggle:
        kind = wx.ITEM_CHECK
    return self.InsertTool(pos, toolId, "", bitmap, wx.NullBitmap, kind, shortHelpString, longHelpString)
...
def _ToolBar_InsertLabelTool(
    ...
    Old style method to insert a tool in the toolbar.
    """
    return self.InsertTool(pos, id, label, bitmap, bmpDisabled, kind, shortHelp, longHelp, clientData)
...
TreeItemData = deprecated(
    TreeItemData, "The TreeItemData class no longer exists, just pass your object directly to the tree instead."
)
    ...
    class ColourPickerCtrl(PickerBase):
        ...
        class ColourPickerButton(BitmapButton):
            def __init__(
                ...
                wx.BitmapButton.__init__(self, parent, id, wx.Bitmap(1, 1), pos, size, style, validator, name)
            ...
            def _makeBitmap(self):
                ...
                if self.HasFlag(CLRP_SHOW_LABEL):
                    ...
                    dc.DrawText(bg.GetAsString(wx.C2S_HTML_SYNTAX), (width - w) / 2, (height - h) / 2)
                return bmp
        ...
        def __init__(
            ...
            widget = ColourPickerCtrl.ColourPickerButton(self, -1, colour, style=self.GetPickerStyle(style))
...
class CallLater(typing.Generic[_P, _T]):
    ...
    def Start(self, millis: int | None = None, *args: _P.args, **kwargs: _P.kwargs) -> None:
        """
        (Re)start the timer
