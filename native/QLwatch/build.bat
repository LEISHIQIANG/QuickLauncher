...
if exist "%VSWHERE%" (
    for /f "usebackq delims=" %%r in (`"%VSWHERE%" -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath`) do (
        if not defined VCPATH ( for /f "delims=" %%i in ('dir /b /ad /o-n "%%~r\VC\Tools\MSVC" 2^>nul') do ( if not defined VCPATH if exist "%%~r\VC\Tools\MSVC\%%i\bin\Hostx64\x64\cl.exe" set "VCPATH=%%~r\VC\Tools\MSVC\%%i" ) )
    )
...
echo Building QLwatch.dll...
cl.exe /LD /O2 /EHsc /std:c++17 /utf-8 QLwatch.cpp /FeQLwatch.dll /link /implib:QLwatch.lib kernel32.lib user32.lib
if errorlevel 1 ( echo ERROR: build failed. && exit /b 1 )
del QLwatch.obj QLwatch.exp QLwatch.lib 2>nul
echo Build succeeded: QLwatch.dll
popd
exit /b 0
