...
class TrayApp(
    ...
    def _on_system_dpi_changed(self, new_scale_percent: int):
        ...
        Global UI scale is a user/application preference, not per-monitor
        state.  Popup and config windows may legitimately move between
        monitors with different DPI, but that must not rewrite
        ``settings.ui_scale_percent`` or reopen the UI at a different size.
        """
        from ui.utils.ui_scale import MAX_SCALE_PERCENT, MIN_SCALE_PERCENT

        try:
            recommended = int(new_scale_percent)
            current = getattr(
            ...
            logger.debug("读取系统 DPI/当前 UI 缩放失败: %s", exc, exc_info=True)
            return

        if recommended < MIN_SCALE_PERCENT or recommended > MAX_SCALE_PERCENT:
            logger.warning(
                "Ignoring WM_DPICHANGED recommended scale outside supported range: %d%% (current UI scale=%d%%)",
                recommended,
                current,
            )
            return
        ...
        logger.info(
            "WM_DPICHANGED recommended scale=%d%%; keeping application UI scale=%d%%",
            recommended,
            current,
        )
