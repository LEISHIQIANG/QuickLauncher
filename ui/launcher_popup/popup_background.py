    ...
    def _load_bg_task(params):
        ...
        try:
            ...
            if blur_level > 0:
                ...
                image = image.scaled(
                    small_w, small_h, QtCompat.KeepAspectRatioByExpanding, QtCompat.SmoothTransformation
                )

                # 2. 放大回目标尺寸 (KeepAspectRatioByExpanding 保证铺满)
                image = image.scaled(
                    target_w, target_h, QtCompat.KeepAspectRatioByExpanding, QtCompat.SmoothTransformation
                )
