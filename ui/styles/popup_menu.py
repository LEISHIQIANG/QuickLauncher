"""PopupMenu Qt widget extracted from ui.styles.style (W6.3 split).

The :class:`PopupMenu` is a themed right-click menu with rounded
corners, optional acrylic blur, and a self-contained show/close
lifecycle. It does not depend on :class:`Colors` or
:class:`StyleSheet` — its theme is provided by the caller.
"""
...
def _app():
    """Resolve :class:`QApplication` from this module's import.

    Tests monkey-patch :attr:`QApplication` on this module to install
    fake screen objects; the helper returns the live attribute so the
    patches are honored without creating a static import edge back to
    :mod:`ui.styles.style`.
    """
    return QApplication
...
def _app_cursor():
    """Resolve :class:`QCursor` from this module's import (see :func:`_app`)."""
    return QCursor
...
class PopupMenu(QWidget):
    ...
    def _hide_if_pointer_outside(self):
        try:
            local_pos = self.mapFromGlobal(_app_cursor().pos())
    ...
    def _refresh_action_hover(self, button):
        ...
        try:
            button.update()
            dirty_rect = button.geometry().adjusted(-sp(4), -sp(4), sp(4), sp(4))
    ...
    def _surface_colors(theme: str, blur_applied: bool, *, win10: bool, win11: bool):
        ...
        from ui.styles import design_tokens

        if blur_applied:
            if theme == "dark":
                bg = QColor("#1f1f23")
                bg.setAlpha(120)
                border = design_tokens.border("dark", "separator")
                border.setAlpha(38)
                return bg, border
            bg = QColor("#ffffff")
            bg.setAlpha(120)
            border = design_tokens.border("light", "separator")
            border.setAlpha(20)
            return bg, border
        if theme == "dark":
            bg = design_tokens.surface("dark", "bg_chrome")
            bg.setAlpha(220)
            border = design_tokens.border("dark", "separator")
            border.setAlpha(int(0.1 * 255))
            return bg, border
        bg = design_tokens.surface("light", "bg_chrome")
        bg.setAlpha(220)
        border = design_tokens.border("light", "separator")
    ...
    def paintEvent(self, event):
        ...
        def _set_hint(p, h, v=True):
            return p.setRenderHint(h, v)

        painter = QPainter(self)
        _set_hint(painter, QtCompat.Antialiasing)
        _set_hint(painter, QtCompat.HighQualityAntialiasing)
        _set_hint(painter, QtCompat.TextAntialiasing)
        _set_hint(painter, QtCompat.SmoothPixmapTransform)

        try:
            if PopupMenu._CM_SOURCE is None:
                cm_source = QPainter.CompositionMode.CompositionMode_Source
                cm_over = QPainter.CompositionMode.CompositionMode_SourceOver
                PopupMenu._CM_SOURCE = cm_source
                PopupMenu._CM_OVER = cm_over
            else:
                cm_source = PopupMenu._CM_SOURCE
                cm_over = PopupMenu._CM_OVER
        except Exception:
        ...
        rect = snap_rect(QRectF(self.rect()))
