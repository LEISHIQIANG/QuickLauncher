...
class IconGrid(QWidget):
    ...
    def _setup_ui(self):
        ...
        # The grid container owns its own symmetric edge padding.  Extra bottom-only
        # layout margins make the first and last rows clip differently while scrolling.
        ...
        self.add_folder_btn = QPushButton(tr("新建分类"))
        self.add_folder_btn.setFixedHeight(sp(36))
        self.add_folder_btn.clicked.connect(self.add_folder_requested.emit)
        self.add_folder_btn.setVisible(False)
        btn_layout.addWidget(self.add_folder_btn, 1)
    ...
    def set_category_add_visible(self, visible: bool):
        """Keep category creation in the bottom action row while tabs are shown above."""
        self.add_folder_btn.setVisible(visible)

    def set_column_count(self, columns: int):
        """Reflow without changing the established icon-cell scale or spacing."""
    ...
    def _get_cell_size(self):
        # 分栏框宽度减去左右各 10px 边距，按当前列数计算单元格尺寸。
        # 使用 sp(20) 确保边距随 UI 缩放同步增长
        w = self.grid_area.width()
        if w <= 0:
            w = 400
        return max(sp(64), (w - sp(20)) // self._columns)
    ...
    def _place_icons(self, animate=False):
        ...
        for i, widget in enumerate(self.icon_widgets):
            ...
            if widget.width() != cell or widget.height() != cell:
                widget.cell_size = cell
                widget.setFixedSize(cell, cell)
        ...
        rows = (len(self.icon_widgets) + self._columns - 1) // self._columns
        self.container.setMinimumHeight(pad_y + rows * cell + pad_y)
    ...
    def dragMoveEvent(self, event):
        if event.mimeData().hasFormat("application/x-shortcut-id"):
            ...
            col = max(0, min(self._columns - 1, (pos_in_container.x() - pad_x) // cell))
            row = max(0, (pos_in_container.y() - pad_y) // cell)
            target_idx = row * self._columns + col
