...
class TitleBar(QWidget):
    ...
    layout_toggle_requested = pyqtSignal()
    ...
    def _setup_ui(self):
        ...
        # 分类栏布局切换：左侧栏 / 浏览器式顶部标签栏
        self.layout_toggle_btn = QPushButton()
        self.layout_toggle_btn.setFixedSize(sp(32), sp(32))
        self.layout_toggle_btn.setCursor(QtCompat.PointingHandCursor)
        install_tooltip(self.layout_toggle_btn, tr("切换分类栏布局"))
        self.layout_toggle_btn.clicked.connect(self.layout_toggle_requested.emit)
        self._layout_toggle_icon_paths = {
            "dark": os.path.join(base_dir, "assets", "system_icons", "layout-switch-dark.svg"),
            "light": os.path.join(base_dir, "assets", "system_icons", "layout-switch-light.svg"),
        }
        self._apply_layout_toggle_icon("dark")
        self.layout_toggle_btn.setStyleSheet(
            ...
            QPushButton:hover { background-color: rgba(128, 128, 128, 0.1); color: #ffffff; }
            QPushButton:pressed { background-color: rgba(128, 128, 128, 0.18); color: #ffffff; }
        """
            )
        )
    ...
    def set_theme(self, theme: str):
        ...
        self.layout_toggle_btn.setStyleSheet(btn_base_style)
        self.settings_btn.setStyleSheet(btn_base_style + scale_qss("QPushButton { font-size: 18px; }"))
        self._apply_layout_toggle_icon(theme)
    ...
    def _apply_layout_toggle_icon(self, theme: str):
        """Load the matching custom SVG rather than relying on a font glyph."""
        icon_path = self._layout_toggle_icon_paths.get(theme, self._layout_toggle_icon_paths["dark"])
        if os.path.exists(icon_path):
            self.layout_toggle_btn.setIcon(QIcon(icon_path))
            self.layout_toggle_btn.setIconSize(QSize(sp(18), sp(18)))
    ...
    def set_mode(self, is_settings):
        ...
        self.layout_toggle_btn.setVisible(not is_settings)
        self.retranslate_ui()
