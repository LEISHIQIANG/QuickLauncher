...
class ConfigWindow(QMainWindow):
    ...
    # 布局常量
    SIDEBAR_FOLDER_PANEL_WIDTH = 160  # base value, scaled at usage
    # SETTINGS_PANEL_WIDTH = 220  # Removed
    ICON_WIDGET_SIZE = 65  # 调整为 65，对应 IconWidget 的正方形尺寸
    SIDEBAR_ICON_COLS = 6
    TOP_TAB_ICON_COLS = 8
    ICON_COLS = SIDEBAR_ICON_COLS  # Compatibility alias for sidebar geometry.
    ...
    def _calculate_window_size(self) -> tuple[int, int]:
        icon_grid_width = self._icon_grid_content_width()
        # Layout switching only changes the category bar position, never the outer window size.
        total_width = sp(self.SIDEBAR_FOLDER_PANEL_WIDTH) + icon_grid_width
        return total_width, sp(560)

    def _icon_grid_content_width(self) -> int:
        return (
    ...
    def _read_config_layout_mode(self) -> str:
        try:
            return "top_tabs" if self.data_manager.get_settings().config_layout_mode == "top_tabs" else "sidebar"
        except Exception:
            return "sidebar"
    ...
    def _setup_ui(self):
        ...
        self.title_bar.layout_toggle_requested.connect(self._toggle_config_layout)
        ...
        self.launcher_layout = QHBoxLayout(launcher_widget)
        self.launcher_layout.setContentsMargins(0, 0, 0, 0)
        self.launcher_layout.setSpacing(0)
        ...
        self.icon_grid.add_folder_requested.connect(self.folder_panel._add_folder)

        # Widgets are added once; layout switching below only changes their direction.
        self.launcher_layout.addWidget(self.folder_panel)
        self.launcher_layout.addWidget(self.icon_grid, 1)
        self._apply_config_layout(self._config_layout_mode, persist=False)
        ...
        self._update_status_bar_margin()
    ...
    def _update_status_bar_margin(self):
        left_margin = sp(self.SIDEBAR_FOLDER_PANEL_WIDTH) + sp(4) if self._config_layout_mode == "sidebar" else sp(4)
        self.status_bar.setContentsMargins(left_margin, 0, 0, 0)

    def _toggle_config_layout(self):
        target = "top_tabs" if self._config_layout_mode == "sidebar" else "sidebar"
        self._animate_config_layout_change(target)

    def _animate_config_layout_change(self, target: str):
        """Animate only the moving layout elements; the window backdrop stays completely still."""
        if self._layout_transition_animation is not None:
            return
        self.title_bar.layout_toggle_btn.setEnabled(False)
        moving_widgets = (self.folder_panel, self.icon_grid)
        start_rects = {widget: QRect(widget.geometry()) for widget in moving_widgets}

        # Let QBoxLayout calculate the destination first, then animate the two existing
        # widgets back into that geometry. Their parent and the window frame never change.
        self._apply_config_layout(target, persist=True)
        self.launcher_layout.activate()
        end_rects = {widget: QRect(widget.geometry()) for widget in moving_widgets}

        animation_group = QParallelAnimationGroup(self)
        for widget in moving_widgets:
            start_rect = start_rects[widget]
            end_rect = end_rects[widget]
            if start_rect == end_rect:
                continue
            widget.setGeometry(start_rect)
            animation = QPropertyAnimation(widget, b"geometry", animation_group)
    ...
    def _apply_config_layout(self, mode: str, *, persist: bool):
        """Move the existing category panel without recreating its selection or drag state."""
        ...
        if mode == "top_tabs":
            ...
            self.icon_grid.setMinimumWidth(0)
            self.icon_grid.setMaximumWidth(16777215)
            self.icon_grid.set_category_add_visible(True)
            ...
            self.icon_grid.setMinimumWidth(0)
            self.icon_grid.setMaximumWidth(16777215)
            self.icon_grid.set_category_add_visible(False)
            self.icon_grid.set_column_count(self.SIDEBAR_ICON_COLS)
            self.launcher_layout.setAlignment(self.icon_grid, Qt.Alignment())
        self._update_status_bar_margin() if hasattr(self, "status_bar") else None
        total_width, total_height = self._calculate_window_size()
        self.setFixedSize(total_width, total_height)
        if persist:
            self.data_manager.update_settings(config_layout_mode=mode)
