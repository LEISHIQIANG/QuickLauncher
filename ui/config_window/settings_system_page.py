...
class SettingsSystemPageMixin:
    def _setup_system_page(self, page):
        ...
        self.auto_start_cb = SwitchButton("开机自动启动")
        ...
        self.hide_tray_cb = SwitchButton("隐藏托盘图标")
        ...
        self.sleep_mode_cb = SwitchButton("10秒无操作后轻睡眠")
        install_tooltip(self.sleep_mode_cb, "无操作一段时间后进入低占用状态，下一次中键立即唤醒")
        self.sleep_mode_cb.stateChanged.connect(self._on_sleep_mode_changed)
        layout.addWidget(self.sleep_mode_cb)

        self.disable_logging_cb = SwitchButton("关闭日志")
        ...
        self.debug_log_cb = SwitchButton("开启DEBUG日志")
        ...
        self.custom_sort_radio = QRadioButton("自定义排序")
        self.smart_sort_radio = QRadioButton("智能排序")
        install_tooltip(self.custom_sort_radio, "按你拖拽调整的顺序显示，不会删除智能排序结果")
        install_tooltip(self.smart_sort_radio, "按使用次数和最近使用时间显示，保留自定义排序可随时切回")
        ...
        for title, tooltip, handler in tool_buttons:
            ...
            install_tooltip(button, tooltip)
    ...
    def _show_utility_window(self, attr_name, factory, refresh_method=None):
        ...
        try:
            ...
            ThemedMessageBox.warning(self, "打开失败", f"无法打开工具窗口:\n{e}")
            return None
