...
class FolderPanel(QWidget):
    ...
    # Each layout owns a separate measurement set; do not share visual dimensions.
    SIDEBAR_ROOT_MARGINS = (8, 8, 8, 1)
    SIDEBAR_LAYOUT_SPACING = 8
    SIDEBAR_LIST_MARGINS = (8, 4, 8, 8)
    SIDEBAR_LIST_SPACING = 8
    SIDEBAR_ADD_BUTTON_HEIGHT = 36
    TOP_TAB_ROOT_MARGINS = (8, 3, 8, 3)
    TOP_TAB_LAYOUT_SPACING = 0
    TOP_TAB_LIST_MARGINS = (6, 3, 6, 3)
    TOP_TAB_LIST_SPACING = 6
    TOP_TAB_ITEM_SPACING = 0
    TOP_TAB_HEIGHT = 56

    def __init__(self, data_manager: DataManager):
        super().__init__()
        self.data_manager = data_manager
        self._layout_mode = "sidebar"
        self._setup_ui()
        self._load_folders()
    ...
    def _setup_ui(self):
        ...
        layout.setContentsMargins(*(sp(value) for value in self.SIDEBAR_ROOT_MARGINS))
        layout.setSpacing(sp(self.SIDEBAR_LAYOUT_SPACING))

        self.list_frame = QFrame()
        self.list_frame.setObjectName("folderListFrame")
        list_frame_layout = QVBoxLayout(self.list_frame)
        list_frame_layout.setContentsMargins(*(sp(value) for value in self.SIDEBAR_LIST_MARGINS))
        list_frame_layout.setSpacing(sp(self.SIDEBAR_LIST_SPACING))
        ...
        self.folder_list.currentItemChanged.connect(self._sync_selection_scroll)
        ...
        self.add_btn.setFixedHeight(sp(self.SIDEBAR_ADD_BUTTON_HEIGHT))
    ...
    def set_layout_mode(self, mode: str):
        """Switch between the traditional side list and a browser-like top tab strip."""
        mode = "top_tabs" if mode == "top_tabs" else "sidebar"
        if mode == self._layout_mode:
            return
        self._layout_mode = mode
        root_layout = self.layout()
        list_layout = self.list_frame.layout()
        if root_layout is None or list_layout is None:
            return

        if mode == "top_tabs":
            root_layout.setContentsMargins(*(sp(value) for value in self.TOP_TAB_ROOT_MARGINS))
            ...
            self.folder_list.setWrapping(False)
            self.folder_list.setHorizontalScrollBarPolicy(QtCompat.ScrollBarAlwaysOff)
            self.folder_list.setVerticalScrollBarPolicy(QtCompat.ScrollBarAlwaysOff)
            self.folder_list.lock_tab_strip_vertical_scroll()
            self.add_btn.setVisible(False)
            self.setFixedHeight(sp(self.TOP_TAB_HEIGHT))
        else:
            root_layout.setContentsMargins(*(sp(value) for value in self.SIDEBAR_ROOT_MARGINS))
        ...
        self._apply_item_layout_mode()
        self.folder_list.doItemsLayout()
        QTimer.singleShot(0, self._fit_sidebar_item_widths)

    @property
    def layout_mode(self) -> str:
        return self._layout_mode

    def _apply_item_layout_mode(self):
        compact_tabs = self._layout_mode == "top_tabs"
        for row in range(self.folder_list.count()):
            ...
            if isinstance(widget, FolderItemWidget):
                widget.set_compact_tab_mode(compact_tabs)
                item.setSizeHint(widget.sizeHint())

    def _fit_sidebar_item_widths(self):
        """Let list-mode rows use the full sidebar width so names are not prematurely clipped."""
        if self._layout_mode != "sidebar":
            return
        available_width = self.folder_list.viewport().width()
        ...
        for row in range(self.folder_list.count()):
            ...
            if isinstance(widget, FolderItemWidget):
                ...
                item.setSizeHint(QSize(available_width, size.height()))
        self.folder_list.doItemsLayout()

    def _sync_selection_scroll(self, current: QListWidgetItem | None, _previous: QListWidgetItem | None = None):
        """Ensure selection and list scrolling remain in sync in either orientation."""
        if current is None:
            return
        QTimer.singleShot(0, lambda item=current: self._scroll_selected_item_into_view(item))

    def _scroll_selected_item_into_view(self, item: QListWidgetItem):
        if item is not self.folder_list.currentItem():
            return
        self.folder_list.scrollToItem(item, self.folder_list.PositionAtCenter)
    ...
    def rescale_ui(self):
        ...
        for row in range(self.folder_list.count()):
            ...
            if isinstance(widget, FolderItemWidget):
                item.setSizeHint(widget.sizeHint())
        self.apply_theme(self._get_current_theme())
        # Reapply the active orientation after the base scale metrics above.
        active_mode = self._layout_mode
        self._layout_mode = ""
        self.set_layout_mode(active_mode)
    ...
    def _load_folders(self):
        ...
        for folder in self.data_manager.data.folders:
            ...
            widget.set_compact_tab_mode(self._layout_mode == "top_tabs")
            widget.item = item
