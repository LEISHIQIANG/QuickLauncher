...
from ui.config_window.support_dialog import _support_image_path, _rounded_pixmap
...
class WobblyCoffeeCup(QLabel):
    ...
    def spawn_heart(self, is_auto=False):
        ...
        fe = FloatingEmoji(emoji, global_screen_pos, None, is_auto=is_auto)
...
class SettingsSupportPageMixin:
    ...
    def _update_support_page_theme(self, page, theme):
        ...
        if hasattr(self, '_support_title_lbl'):
            self._support_title_lbl.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {title_color}; background: transparent; border: none;")
        if hasattr(self, '_support_desc_lbl'):
            self._support_desc_lbl.setStyleSheet(f"font-size: 11px; color: {desc_color}; line-height: 1.4; background: transparent; border: none;")
        if hasattr(self, '_reaction_label'):
            self._reaction_label.setStyleSheet(f"font-size: 11px; color: {desc_color}; background: transparent; border: none;")
    ...
    def _on_drink_clicked(self, name, price):
        ...
        if sender_card:
            ...
            for _ in range(4):
                fe = FloatingEmoji(random.choice(emojis), global_pos, None, is_auto=False)
