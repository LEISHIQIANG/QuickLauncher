...
def test_config_window_create_close_smoke(qapp, tmp_path, monkeypatch):
    ...
    try:
        ...
        assert not window.title_bar.layout_toggle_btn.icon().pixmap(QSize(18, 18)).isNull()
        window.folder_panel._fit_sidebar_item_widths()
        current_item = window.folder_panel.folder_list.currentItem()
        assert current_item is not None
        assert current_item.sizeHint().width() == window.folder_panel.folder_list.viewport().width()
    finally:
        window.close()
        qapp.processEvents()
...
def test_config_window_category_layout_switch_persists_and_reuses_panel(qapp, tmp_path, monkeypatch):
    import ui.config_window.main_window as main_window_mod
    from ui.config_window.main_window import ConfigWindow
    ...
    window = ConfigWindow(manager)
    try:
        panel = window.folder_panel
        original_size = window.size()
        window._apply_config_layout("top_tabs", persist=True)
        assert window.folder_panel is panel
        assert panel.layout_mode == "top_tabs"
        assert manager.get_settings().config_layout_mode == "top_tabs"
        assert panel.maximumHeight() == panel.minimumHeight()
        assert panel.minimumHeight() == sp(56)
        assert window.size() == original_size
        assert not window.icon_grid.add_folder_btn.isHidden()
        assert window.icon_grid._columns == 8
        assert panel.folder_list.horizontalScrollBarPolicy() == QtCompat.ScrollBarAlwaysOff

        window._apply_config_layout("sidebar", persist=True)
        assert window.folder_panel is panel
        assert panel.layout_mode == "sidebar"
        assert manager.get_settings().config_layout_mode == "sidebar"
        assert window.size() == original_size
        assert window.icon_grid.add_folder_btn.isHidden()
        assert window.icon_grid._columns == 6
        assert panel.folder_list.verticalScrollBarPolicy() == QtCompat.ScrollBarAlwaysOff
    finally:
        window.close()
        qapp.processEvents()
...
def test_config_window_layout_toggle_button_visibility(qapp, tmp_path):
    from ui.config_window.main_window import ConfigWindow

    manager = _SmokeManager(tmp_path)
    window = ConfigWindow(manager)
    try:
        # By default, we are in launcher/config mode (is_settings = False)
        # The layout toggle button should be visible
        assert not window.title_bar.layout_toggle_btn.isHidden()

        # Test toggling layout via clicking the layout toggle button
        initial_mode = manager.get_settings().config_layout_mode
        window.title_bar.layout_toggle_btn.click()
        qapp.processEvents()

        # Verify mode has toggled and is persisted
        new_mode = manager.get_settings().config_layout_mode
        assert new_mode != initial_mode

        # Navigate to settings page
        window._show_settings()
        # The layout toggle button should be hidden
        assert window.title_bar.layout_toggle_btn.isHidden()

        # Go back to launcher page
        window._show_launcher()
        # The layout toggle button should be visible again
        assert not window.title_bar.layout_toggle_btn.isHidden()
    finally:
        window.close()
        qapp.processEvents()
