...
class TestTextProcessors:
    ...
    def test_text_contains_case_sensitive(self):
        result = execute_extra_text_processor("text_contains", {"text": "Hello World", "substring": "hello", "case_sensitive": "true"})
        assert result.success is True
        assert _output_bool(result) == "false"

    def test_text_contains_case_insensitive(self):
        result = execute_extra_text_processor("text_contains", {"text": "Hello World", "substring": "hello", "case_sensitive": "false"})
        assert result.success is True
    ...
    def test_text_regex_replace(self):
        result = execute_extra_text_processor("text_regex_replace", {"text": "foo123bar456", "pattern": r"\d+", "replacement": "X"})
        assert result.success is True
...
class TestMathProcessors:
    ...
    @pytest.mark.parametrize("processor_id,number", [
        ("math_abs", ""),
        ("math_ceil", ""),
        ("math_floor", ""),
    ])
...
class TestDatetimeProcessors:
    ...
    def test_datetime_format(self):
        result = execute_extra_datetime_processor("datetime_format", {"datetime": "2024-06-15 10:30:00", "format": "%Y/%m/%d"})
        assert result.success is True
        assert _output(result) == "2024/06/15"

    def test_datetime_add_days(self):
        result = execute_extra_datetime_processor("datetime_add", {"datetime": "2024-01-01 00:00:00", "days": "10", "format": "%Y-%m-%d"})
        assert result.success is True
        assert _output(result) == "2024-01-11"

    def test_datetime_diff_seconds(self):
        result = execute_extra_datetime_processor("datetime_diff", {
...
class TestSystemProcessors:
    ...
    def test_env_get_missing_with_default(self):
        result = execute_extra_system_processor("env_get", {"key": "_NONEXISTENT_CHAIN_VAR_12345", "default": "fallback"})
        assert result.success is True
        assert _output(result) == "fallback"
...
class TestDataProcessors:
    ...
    def test_dict_merge(self):
        import json
        result = execute_extra_data_processor("dict_merge", {
            "a": json.dumps({"x": 1}),
            "b": json.dumps({"y": 2}),
        })
        assert result.success is True
    ...
    def test_str_truncate(self):
        result = execute_extra_data_processor("str_truncate", {"text": "hello world", "max_length": "8", "suffix": "..."})
        assert result.success is True
        assert _output(result) == "hello..."
...
class TestExtraProcessorDispatch:
    ...
    @pytest.mark.parametrize("processor_id,values,expected_check", [
        # Text processors
    ...
    @pytest.mark.parametrize("processor_id", [
