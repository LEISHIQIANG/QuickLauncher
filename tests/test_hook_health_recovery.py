...
def test_sync_special_apps_to_hook_updates_monitor_and_mouse_hook():
    calls = []
    tray = SimpleNamespace(
        _sleeping=False,
        mouse_hook=object(),
        _update_special_app_monitors=lambda **kwargs: calls.append(("monitor", kwargs)),
        _apply_mouse_hook_settings=lambda: calls.append(("apply",)),
    )

    HooksMixin._sync_special_apps_to_hook(tray)

    assert calls == [("monitor", {"reset_state": True}), ("apply",)]


def test_sync_special_apps_to_hook_stops_monitor_while_sleeping():
    calls = []
    ...
    tray = SimpleNamespace(
        _sleeping=True,
        mouse_hook=None,
        _process_check_timer=Timer(),
        _special_app_monitors_active=True,
        _reset_special_app_monitor_state=lambda: calls.append(("reset",)),
    )

    HooksMixin._sync_special_apps_to_hook(tray)

    assert calls == [("reset",), ("stop_timer",)]
    assert tray._special_app_monitors_active is False
