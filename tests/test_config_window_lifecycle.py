...
def test_system_dpi_change_does_not_rewrite_global_ui_scale():
    from ui.tray_app import TrayApp
    ...
    tray.data_manager = SimpleNamespace(
        get_settings=lambda: SimpleNamespace(ui_scale_percent=110),
        update_settings=lambda **kwargs: updates.append(kwargs),
    )
    tray.apply_ui_scale_and_reopen_config = lambda percent: reopen_calls.append(percent)

    TrayApp._on_system_dpi_changed(tray, 250)

    assert updates == []
    assert reopen_calls == []


def test_system_dpi_change_rejects_pointer_sized_scale_values():
    from ui.tray_app import TrayApp
    ...
    TrayApp._on_system_dpi_changed(tray, 45583)
