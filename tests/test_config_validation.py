...
def test_validate_chain_type_is_invalid():
    """chain 类型已移除，应产生 invalid_shortcut_type issue。"""
    data = AppData().to_dict()
    data["folders"][0]["items"] = [
        {"id": "c1", "name": "My Chain", "type": "chain", "chain_steps": [{"shortcut_id": "ref"}]},
    ]
    issues = validate_app_data_dict(data)
    issue_texts = [str(i) for i in issues]
    assert any("invalid_shortcut_type" in t for t in issue_texts)
