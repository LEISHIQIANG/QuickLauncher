...
class AppSettings:
    ...
    # 配置窗口分类栏布局：sidebar=左侧栏，top_tabs=顶部标签栏
    config_layout_mode: str = "sidebar"
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "config_layout_mode": self.config_layout_mode,
        }
        ...
        for key, value in data.items():
            if hasattr(settings, key):
                ...
                if key == "sort_mode" and value not in ("custom", "smart"):
                    value = "custom"
                if key == "config_layout_mode" and value not in ("sidebar", "top_tabs"):
                    value = "sidebar"
