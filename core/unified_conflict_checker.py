...
class UnifiedConflictChecker:
    ...
    def check_hotkey(self, hotkey_str: str, context: dict = None) -> ConflictReport:
        ...
        if "shortcuts" in context:
            ...
            internal_conflicts = self._check_internal_hotkey_duplicates(
                hotkey_str, shortcuts, exclude_id
            )
            conflicts.extend(internal_conflicts)

        # 3. 检查与弹窗触发冲突
        if "settings" in context:
            trigger_conflicts = self._check_hotkey_vs_popup_trigger(
                hotkey_str, context["settings"]
            )
    ...
    def _check_system_conflict(self, hotkey_str: str) -> Conflict | None:
        ...
        if is_conflict:
            return Conflict(
                type="system",
                source="Windows系统/常用软件",
                details=msg
            )
        return None

    def _check_internal_hotkey_duplicates(self, hotkey_str: str, shortcuts: list,
                                          exclude_id: str = None) -> list[Conflict]:
        """检查快捷键是否与其他快捷方式重复"""
        conflicts = []
    ...
    def check_trigger_config(self, mode: str, keys: list[str], button: str,
                            modifiers: list[str]) -> ConflictReport:
        """检查触发配置冲突
            ...
            if not keys:
                ...
                if not modifiers:
                    conflicts.append(Conflict(
                        type="validation",
                        source="触发配置",
                        details="键盘触发必须配合修饰键（Ctrl/Shift/Win）"
                    ))
    ...
    def _button_name(button: str) -> str:
        """按键显示名称"""
        mapping = {"left": "左键", "right": "右键", "middle": "中键",
                  "x1": "侧键后", "x2": "侧键前"}
