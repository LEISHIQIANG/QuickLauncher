...
SLASH_COMMANDS = [
    ...
    SlashCommand(
        "shortcut-health",
        ["shortcut-health", "health", "icons", "图标检查", "图标诊断", "诊断图标"],
        "检查图标、路径和命令风险",
        "internal",
    ...
    SlashCommand(
        "clean-icons",
        ["clean-icons", "icon-cache", "clear-icons", "清理图标", "图标缓存"],
        "立即清理图标缓存",
        "internal",
        "clean_icon_cache",
    ...
    SlashCommand(
        "clean-cache",
        ["clean-cache", "cache-clean", "clear-cache", "清理缓存", "缓存清理"],
        "清理项目目录下未使用的临时缓存",
        "internal",
        "clean-cache",
    ...
    SlashCommand(
        "reload-hooks",
        ["reload-hooks", "hooks", "reinstall-hooks", "重装钩子", "钩子"],
        "重装鼠标和键盘钩子",
        "internal",
        "reload_hooks",
    ...
    SlashCommand(
        "data-dir",
        ["data-dir", "app-data", "config-dir", "数据目录", "配置目录"],
        "打开配置数据目录",
        "internal",
        "open_data_dir",
    ...
    SlashCommand(
        "install-dir",
        ["install-dir", "program-dir", "project-dir", "安装目录", "项目目录", "软件目录"],
        "打开软件安装目录",
        "internal",
        "open_install_dir",
    ...
    SlashCommand(
        "config-file",
        ["config-file", "config_file", "data-json", "data.json", "配置文件"],
        "打开 data.json 配置文件",
        "internal",
        "open_config_file",
    ...
    SlashCommand(
        "icons-dir",
        ["icons-dir", "icon-dir", "user-icons", "图标目录", "用户图标"],
        "打开用户图标目录",
        "internal",
        "open_icons_dir",
    ...
    SlashCommand(
        "history-dir",
        ["history-dir", "config-history-dir", "历史目录", "快照目录"],
        "打开配置历史快照目录",
        "internal",
        "open_history_dir",
    ...
    SlashCommand(
        "auto-backups",
        ["auto-backups", "backup-dir", "backups-dir", "自动备份", "备份目录"],
        "打开自动备份目录",
        "internal",
    ...
    SlashCommand(
        "error-log",
        ["error-log", "error.log", "log-file", "错误日志"],
        "打开 error.log 或日志目录",
        "internal",
        "open_error_log",
    ...
    SlashCommand(
        "path-audit",
        ["path-audit", "path", "env-path", "环境变量", "path体检"],
    ...
    SlashCommand(
        "explorer",
        ["explorer", "重启资源管理器", "restart-explorer"],
...
def _convert_to_slash_command(cmd_def) -> SlashCommand | None:
    ...
    try:
        ...
        if cmd_def.params:
            hints = " ".join(f"[{p.name}]" if p.required else f"[{p.name}?]" for p in cmd_def.params)
