"""Audit motion usage: animation primitives vs. ``QTimer.singleShot`` chains.

Per §1.5 and §4.10.3 of ``UI_OPTIMIZATION_PLAN.md`` the project prefers
:class:`QPropertyAnimation` and :mod:`ui.utils.animations` semantic
helpers over ``QTimer.singleShot``-based animation chains. This script
flags ``QTimer.singleShot`` calls that look like animation steps
(``duration`` between 16 ms and 2000 ms and a callable performing a
``setX``/``setY``/``setOpacity`` etc.).

It is a *warning* audit, not a blocking one – many uses of
``QTimer.singleShot`` are legitimate (debounce, debounced search). The
script therefore only counts calls whose duration falls in the
'animation' band (40-2000 ms) and whose callback argument contains a
``setX``/``setY``/``setWindowOpacity``/``setProperty`` call.
...
::
    ...
    python scripts/audit_motion_consistency.py
"""
...
# Match QTimer.singleShot(<int>, ...) – duration_ms first
SINGLE_SHOT_RE = re.compile(r"QTimer\.singleShot\s*\(\s*(\d+)\s*,")
SETTER_RE = re.compile(r"\.set(?:WindowOpacity|Property|Geometry|Pos|Value|Text|Visible|Enabled)\s*\(")
...
def _scan_file(path: Path) -> Iterable[tuple[int, str, int]]:
    ...
    for lineno, line in enumerate(lines, start=1):
        ...
        if duration < 40 or duration > 2000:
            continue
        # Look at the rest of the call (allow multi-line)
        snippet = line
