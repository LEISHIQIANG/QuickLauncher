...
**强大的命令系统。** 支持 CMD、PowerShell、Python、Git Bash 和内置命令，配合参数模板、动态变量、输出捕获和实时结果面板，可以把常用脚本和排障动作沉淀成快捷命令。
...
**插件扩展。** 支持本地插件、`.qlzip` 打包安装、热加载、权限声明、自定义命令和搜索源。内置插件覆盖进程、磁盘、网络、启动项和文本工具等场景。
...
| 命令系统 | CMD / PowerShell / Python / Git Bash、参数模板、实时输出捕获 |
| 动作链 | 多步骤顺序执行，支持步骤间数据传递和取消 |
| 命令变量 | `{{clipboard}}`、`{{date}}`、`{{lan_ip}}`、`{{wan_ip}}`、`{{input}}` 等 |
| 安全预处理 | 语法、语义、安全扫描、业务规则和审计日志五层管道 |
| 外观定制 | 深色/浅色主题、亚克力背景、自定义图片和布局配置 |
| 插件系统 | 权限管理、热加载、`.qlzip` 打包、自定义命令与搜索源 |
| 数据安全 | 原子保存、自动备份、20 个配置快照、配置损坏自动恢复 |
| 自动更新 | GitHub Releases 源、SHA-256 校验、后台下载和静默安装 |
