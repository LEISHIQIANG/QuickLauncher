...
QuickLauncher 是一款面向 Windows 10 / 11 的桌面快速启动与轻量自动化工具。它可以启动应用、打开文件夹和 URL、发送热键、运行命令、批量启动一组目标，并通过本地插件继续扩展能力。

当前源码版本以 [core/version.py](core/version.py) 为准：`1.6.3.3` stable。安装包和便携包自带运行时，普通用户不需要在电脑上安装 Python；只有从源码运行或构建时才需要 CPython 3.12。
...
| 快捷方式 | 7 类：文件 / 应用、文件夹、URL、热键、命令、批量启动、宏录制 |
...
| 插件系统 | `.qlzip` 安装包、热加载、启用/禁用、失败隔离、权限声明、内置命令注册、搜索源和常驻 worker |
| 外观与交互 | 深色/浅色和跟随系统、Acrylic/图片/纯色背景、Win10 阴影、全局 UI 缩放、Dock、固定和拖拽弹窗 |
| 数据安全 | 原子保存、配置历史、自动备份、导入清洗、路径穿越防护、URL 安全抓取、SHA-256 校验 |
...
4. 在设置页添加文件、文件夹、URL、热键、命令或批量启动项。
...
py -3.12 scripts/check_mypy_progress.py
